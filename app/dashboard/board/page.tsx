  ...
  return (
    <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.4 : 1, background: 'var(--s1)', border: '1px solid var(--bd)', borderRadius: 9, padding: '10px 12px', cursor: 'grab', marginBottom: 6, borderLeft: `3px solid ${PRIORITY_COLOR[card.priority] ?? '#4a5d78'}` }}
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {card.assigned_name && <span style={{ fontSize: 10, color: 'var(--bl)', background: 'var(--bl3)', padding: '2px 6px', borderRadius: 4, fontWeight: 600 }}>→ {card.assigned_name}</span>}
      </div>
    </div>
  );
}
...
interface CardModalProps {
  card: Partial<Card>; columns: Column[]; admins: AdminUser[];
  onSave: (d: Partial<Card>) => void; onDelete?: () => void; onClose: () => void;
}

function CardModal({ card, columns, admins, onSave, onDelete, onClose }: CardModalProps) {
  ...
  function save() {
    ...
    onSave({
      ...
      priority, due_date: dueDate || null, column_id: colId,
...
export default function BoardPage() {
  ...
  async function loadAdmins() {
    const users = await fetch('/api/admin/sellers').then(r => r.json()) as AdminUser[];
    setAdmins(users.filter(u => u.role === 'ADMIN' || u.role === 'MANAGER'));
  }

  async function reloadCards() {
    if (!boardId) return;
    const cds = await fetch(`/api/kanban/cards?boardId=${boardId}`).then(r => r.json()) as Card[];
    setCards(cds);
  }

  useEffect(() => { loadBoard(); loadAdmins(); }, []);
  ...
  async function deleteCard() {
    if (!modal?.card.id || !boardId) return;
    await fetch(`/api/kanban/cards/${modal.card.id}`, { method: 'DELETE' });
    setModal(null); setToast('Kort slettet'); reloadCards();
  }
  ...
  async function addColumn() {
    ...
    const cols = await fetch(`/api/kanban/boards/${boardId}/columns`).then(r => r.json()) as Column[];
    setColumns(cols);
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--bg)' }}>
      ...
      <div style={{ padding: '16px 18px', borderBottom: '1px solid var(--bd)', display: 'flex', alignItems: 'center', gap: 12, background: 'var(--s1)', flexShrink: 0 }}>
        <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--t1)' }}>Mit board</div>
        <div style={{ fontSize: 12, color: 'var(--t2)', marginTop: 2 }}>Privat board</div>
      </div>
              ...
              return (
                <div key={col.id} style={{ width: 210, minWidth: 210, flexShrink: 0, display: 'flex', flexDirection: 'column' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                    ...
                    <button onClick={() => setModal({ card: { priority: 'normal' }, colId: col.id })} style={{ background: 'none', color: 'var(--t3)', fontSize: 16, padding: '0 4px', border: 'none', borderRadius: 4 }}>+</button>
                  </div>
                    ...
                    <div id={`col-${col.id}`} style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
                      ...
                      {doneCards.length > 0 && (
                        <div style={{ borderTop: '1px dashed var(--bd)', paddingTop: 6, marginTop: 4 }}>
                          {doneCards.map(card => <CardItem key={card.id} card={card} onClick={() => setModal({ card })} />)}
                        </div>
                      )}
                    </div>
                  </SortableContext>
      ...
      {modal && (
        <CardModal card={modal.card} columns={columns} admins={admins}
