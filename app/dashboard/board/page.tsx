  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
        {card.due_date && (
          <span style={{ fontSize: 10, color: 'var(--t3)', background: 'var(--s2)', padding: '2px 6px', borderRadius: 4 }}>
            📅 {new Date(card.due_date + 'T12:00:00').toLocaleDateString('da-DK', { day: 'numeric', month: 'short' })}
          </span>
        )}
        {card.assigned_name && (
          <span style={{ fontSize: 10, color: 'var(--t3)', background: 'var(--s2)', padding: '2px 6px', borderRadius: 4 }}>
...
function CardModal({ card, columns, onSave, onDelete, onClose }: CardModalProps) {
  ...
  return (
    <div className="modal-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(4px)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      ...
      <div className="modal-box" style={{ background: 'var(--s1)', borderRadius: 13, padding: 24, width: 440, maxWidth: '94vw', boxShadow: '0 40px 80px rgba(0,0,0,0.7)', animation: 'modalIn 0.2s cubic-bezier(0.34,1.56,0.64,1)' }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--t1)', marginBottom: 18 }}>
          {card.id ? 'Rediger kort' : 'Nyt kort'}
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div>
            <label>Titel</label>
            <input value={title} onChange={e => setTitle(e.target.value)} onKeyDown={e => e.key === 'Enter' && save()} autoFocus />
          </div>
          <div>
          ...
          {card.id && (
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', marginBottom: 0 }}>
              ...
              <span style={{ color: 'var(--t2)', fontSize: 12 }}>Marker som færdig</span>
            </label>
          )}
        </div>
        <div style={{ display: 'flex', gap: 8, marginTop: 20, justifyContent: 'flex-end' }}>
          {card.id && onDelete && (
            <button onClick={onDelete} style={{ background: 'var(--re2)', color: 'var(--re)', border: '1px solid var(--re)', borderRadius: 7, padding: '8px 14px', fontSize: 12, marginRight: 'auto' }}>
              Slet
            </button>
          )}
...
export default function BoardPage() {
  ...
  async function saveCard(data: Partial<Card>) {
    ...
    if (modal?.card.id) {
      await fetch(`/api/kanban/cards/${modal.card.id}`, {
        method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data),
      });
      showToast('Kort opdateret');
    } else {
      await fetch('/api/kanban/cards', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...data, board_id: activeBoardId, column_id: modal?.colId ?? columns[0]?.id }),
      });
      showToast('Kort oprettet');
    }
    setModal(null);
  ...
  async function addColumn() {
    ...
    await fetch(`/api/kanban/boards/${activeBoardId}/columns`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--bg)' }}>
      ...
      <div style={{ padding: '16px 18px', borderBottom: '1px solid var(--bd)', display: 'flex', alignItems: 'center', gap: 12, background: 'var(--s1)', flexShrink: 0 }}>
        ...
        {boards.length > 1 && (
          <select value={activeBoardId ?? ''} onChange={e => setActiveBoardId(Number(e.target.value))}
      ...
      {/* Board */}
      <div style={{ flex: 1, overflowX: 'auto', overflowY: 'hidden', scrollSnapType: 'x mandatory' }}>
        <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd}>
          <div style={{ display: 'flex', gap: 10, padding: 18, height: '100%', minWidth: 'max-content' }}>
            {columns.map(col => {
              ...
              return (
                <div key={col.id} id={`col-${col.id}`}
                  style={{ width: 210, minWidth: 210, flexShrink: 0, display: 'flex', flexDirection: 'column', scrollSnapAlign: 'start' }}>
                  {/* Column header */}
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--t1)', display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ width: 8, height: 8, borderRadius: '50%', background: col.color, display: 'inline-block' }} />
                      {col.name}
                      <span style={{ fontSize: 10, color: 'var(--t3)', fontWeight: 400 }}>{allColCards.length}</span>
                    </div>
                    <button
                      onClick={() => setModal({ type: 'card', card: { priority: 'normal' }, colId: col.id })}
                      style={{ background: 'none', color: 'var(--t3)', fontSize: 16, padding: '0 4px', lineHeight: 1, border: 'none', borderRadius: 4 }}
                    >+</button>
                  </div>
                    ...
                    >
                      {colCards.map(card => (
                        <CardItem key={card.id} card={card}
                          onClick={() => setModal({ type: 'card', card })} />
                      ))}
                      {doneCards.length > 0 && (
                  ...
                  <button
                    onClick={() => setModal({ type: 'card', card: { priority: 'normal' }, colId: col.id })}
                    style={{ marginTop: 6, border: '1px dashed var(--bd)', borderRadius: 7, padding: '8px 0', color: 'var(--t3)', fontSize: 12, background: 'transparent', transition: 'all 0.12s', minHeight: 44 }}
            ...
            {/* Add column */}
            <div style={{ width: addingCol ? 210 : 150, minWidth: addingCol ? 210 : 150, flexShrink: 0, scrollSnapAlign: 'start' }}>
              {addingCol ? (
                ...
                <button onClick={() => setAddingCol(true)}
                  style={{ width: '100%', border: '1px dashed var(--bd)', borderRadius: 9, padding: '10px 0', color: 'var(--t3)', fontSize: 12, background: 'transparent', minHeight: 44 }}>
                  + Ny kolonne
                </button>
              )}
            </div>
      ...
      {modal?.type === 'card' && (
        <CardModal
          ...
          columns={columns}
          onSave={saveCard}
          onDelete={modal.card.id ? deleteCard : undefined}
