...
interface Ticket {
  id: number; customer_name: string; subject: string; type: string;
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: 'var(--s1)', borderBottom: '1px solid var(--bd)' }}>
                  {['#', 'Kunde', 'Emne', 'Type', 'Prioritet', 'Status', 'Tildelt', 'Alder', '💬'].map(h => (
            ...
            <div style={{ padding: '16px 18px', borderBottom: '1px solid var(--bd)', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--t3)' }}>{detail.customer_name} · #{detail.id}</div>
                </div>
...
function NewTicketModal({ customers, onClose, onCreated }: { customers: Customer[]; onClose: () => void; onCreated: () => void }) {
  const [form, setForm] = useState({ customer_id: '', subject: '', description: '', type: 'general', priority: 'normal' });
