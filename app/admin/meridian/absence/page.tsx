...
export default function MeridianAbsencePage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [ab, sel] = await Promise.all([
      fetch('/api/absences?company=meridian').then(r => r.json()),
      fetch('/api/admin/sellers').then(r => r.json()),
    ]);
    setAbsences(Array.isArray(ab) ? ab : []);
    const allSellers = Array.isArray(sel) ? sel as Seller[] : [];
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 900 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 700, color: 'var(--t1)', marginBottom: 3 }}>Fravær · Meridian</h1>
          <div style={{ fontSize: 12, color: 'var(--t3)' }}>Ferie, sygdom og andet fravær</div>
        </div>
      ...
      {loading ? (
        ...
        <div style={{ color: 'var(--t3)', fontSize: 13, padding: '40px 0', textAlign: 'center' }}>Ingen fraværsregistreringer for Meridian</div>
      ) : (
          ...
          {absences.map(a => (
            <div key={a.id} style={{ background: 'var(--s1)', border: '1px solid var(--bd)', borderRadius: 10, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--t1)' }}>{a.user_name}</div>
      ...
      {showForm && (
        <div className="modal-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(4px)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          ...
          <div className="modal-box" style={{ background: 'var(--s1)', borderRadius: 13, padding: 24, width: 400, maxWidth: '94vw' }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--t1)', marginBottom: 18 }}>Registrer fravær · Meridian</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div><label>Medarbejder</label><select value={formUserId} onChange={e => setFormUserId(e.target.value)}><option value="">— Vælg —</option>{sellers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
