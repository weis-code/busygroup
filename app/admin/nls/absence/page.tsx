...
export default function NlsAbsencePage() {
  ...
  const load = useCallback(async () => {
    ...
    const [ab, sel] = await Promise.all([
      fetch('/api/absences?company=nls').then(r => r.json()),
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 900 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 700, color: 'var(--t1)', marginBottom: 3 }}>Fravær · NextLevel Sales</h1>
