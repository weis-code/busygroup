  ...
  return (
    <div style={{ display: 'flex', height: '100%', background: 'var(--bg)' }}>
      ...
      <div className={`msg-list${showThread ? ' hidden-mobile' : ''}`} style={{
        width: 260, flexShrink: 0, borderRight: '1px solid var(--bd)',
        ...
        <div style={{ padding: '14px 14px 10px', borderBottom: '1px solid var(--bd)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            <button onClick={() => setShowNewChannel(true)} style={{ background: 'none', border: 'none', color: 'var(--t3)', fontSize: 18, padding: '0 4px', cursor: 'pointer', minHeight: 44, minWidth: 44 }} title="Ny kanal">+</button>
              ...
              {messages.map((msg, i) => {
                ...
                return (
                  <div key={msg.id} style={{ display: 'flex', flexDirection: isOwn ? 'row-reverse' : 'row', alignItems: 'flex-end', gap: 8, marginBottom: 2 }}>
                    {!isOwn && (
                      <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--s3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: 'var(--t2)', flexShrink: 0 }}>
                    ...
                    <div style={{ maxWidth: '70%' }}>
                      ...
                      <div style={{
                        ...
                        padding: '8px 12px', fontSize: 13, color: 'var(--t1)', lineHeight: 1.5, wordBreak: 'break-word',
                      }}>
                        {msg.body}
                      </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 300, overflowY: 'auto' }}>
              {users.filter(u => u.id !== myId).map(u => (
                <button key={u.id} onClick={() => startDm(u.id)}
                  ...
                  <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--s3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: 'var(--t2)', flexShrink: 0 }}>
      ...
      {showNewChannel && (
        <div className="modal-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(4px)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          ...
          <div className="modal-box" style={{ background: 'var(--s1)', borderRadius: 13, padding: 24, width: 360, maxWidth: '94vw', boxShadow: '0 40px 80px rgba(0,0,0,0.7)' }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--t1)', marginBottom: 16 }}>Ny kanal</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                <label>Virksomhed</label>
                <select value={newChannel.company_id} onChange={e => setNewChannel(nc => ({ ...nc, company_id: e.target.value }))}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 16, justifyContent: 'flex-end' }}>
              ...
              <button onClick={createChannel} disabled={!newChannel.name || !newChannel.company_id}
