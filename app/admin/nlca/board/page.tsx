...
export default function NlcaBoardPage() {
  ...
  async function loadBoards() {
    ...
    const co   = data.filter(b => !b.owner_user_id && b.company_slug === 'nlca');
    setBoards(co);
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => { if (activeBoardId) void loadBoard(activeBoardId); }, [activeBoardId]);

  return (
    <BoardCanvas
      boards={boards} activeBoardId={activeBoardId} onBoardChange={setActiveBoardId}
      columns={columns} cards={cards}
      onReload={() => activeBoardId && void loadBoard(activeBoardId)}
