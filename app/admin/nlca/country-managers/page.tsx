...
export default function NlcaCountryManagersPage() {
  const router = useRouter();
  const [managers, setManagers] = useState<CountryManager[]>([]);
  const [showCreate, setShowCreate] = useState(false);
  const [form, setForm] = useState({ name: '', email: '', password: '', country: '', pct: '5' });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');

  const load = useCallback(async () => {
    try {
      const res = await fetch('/api/nlca/country-managers');
      if (res.status === 403) { router.push('/admin/nlca'); return; }
      if (!res.ok) return;
      const data = await res.json() as CountryManager[];
  ...
  async function createManager() {
    ...
    try {
      ...
      const created = await res.json() as CountryManager;
      setManagers(prev => [...prev, created].sort((a, b) => a.country.localeCompare(b.country)));
      setForm({ name: '', email: '', password: '', country: '', pct: '5' });
      setShowCreate(false);
      void load();
    } catch {
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 900 }}>
      <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h1 style={{ fontSize: 18, fontWeight: 700, color: 'var(--t1)', margin: 0, marginBottom: 4 }}>Landsmanagere</h1>
          <div style={{ fontSize: 12, color: 'var(--t3)' }}>NLCA · {managers.length} landsmanagere · 5% cut pr. land</div>
        </div>
        <button
          onClick={() => setShowCreate(true)}
          style={{ background: GREEN, color: '#fff', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}
        >
          + Opret landsmanager
        </button>
      </div>
      ...
      <div style={{ background: 'var(--s1)', border: '1px solid var(--bd)', borderRadius: 12, overflow: 'hidden' }}>
        {managers.length === 0 ? (
          <div style={{ padding: '60px 40px', textAlign: 'center', color: 'var(--t3)', fontSize: 13 }}>
            Ingen landsmanagere endnu. <button onClick={() => setShowCreate(true)} style={{ color: GREEN, background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>Opret den første →</button>
          </div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: 'var(--s2)' }}>
                ...
                <th style={{ padding: '9px 14px', textAlign: 'right', fontWeight: 600, color: 'var(--t2)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Pct</th>
            ...
            <tbody>
              {managers.map((m, i) => (
                <tr key={m.id} style={{ borderTop: i > 0 ? '1px solid var(--bd)' : 'none' }}>
                  ...
                  <td style={{ padding: '11px 14px', color: 'var(--t1)', fontWeight: 600 }}>{m.country}</td>
                  <td style={{ padding: '11px 14px', color: 'var(--t3)', fontSize: 12 }}>{m.email ?? '—'}</td>
                  <td style={{ padding: '11px 14px', textAlign: 'right', color: 'var(--t3)' }}>{Number(m.pct)}%</td>
                  <td style={{ padding: '11px 14px', textAlign: 'right', color: 'var(--t1)' }}>{m.creator_count}</td>
                  <td style={{ padding: '11px 14px', textAlign: 'right', fontWeight: 600, color: GREEN }}>{USD(Number(m.payout_this_month ?? 0))}</td>
      ...
      {showCreate && (
        ...
        >
          <div style={{ background: 'var(--s1)', borderRadius: 13, padding: 28, width: 420, maxWidth: '94vw' }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--t1)', marginBottom: 6 }}>Opret landsmanager</div>
            <div style={{ fontSize: 12, color: 'var(--t3)', marginBottom: 20 }}>Opretter en brugerkonto med rollen NLCA_COUNTRY_MANAGER</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 10 }}>
                ...
                <div>
                  <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--t2)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Pct %</label>
                  <input type="number" min="0" max="100" step="0.01" value={form.pct} onChange={e => setForm(f => ({ ...f, pct: e.target.value }))} style={{ width: '100%', marginTop: 4 }} />
                </div>
              </div>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 20, justifyContent: 'flex-end' }}>
              ...
              <button onClick={() => void createManager()} disabled={!form.name || !form.email || !form.password || !form.country || saving} style={{ background: GREEN, color: '#fff', border: 'none', borderRadius: 7, padding: '8px 16px', fontSize: 12, fontWeight: 600, opacity: saving ? 0.7 : 1 }}>
                {saving ? 'Opretter…' : 'Opret landsmanager'}
              </button>
            </div>
          </div>
        </div>
