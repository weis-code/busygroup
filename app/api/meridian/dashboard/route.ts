...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    let awaitingTicketCount = 0;
    let resolvedTodayCount = 0;
    let latestOpenTickets: Record<string, unknown>[] = [];
    try {
      ...
      const ticketRows      = await sql`
        ...
        LIMIT 5
      `;
      openTicketCount      = Number(openRow?.count ?? 0);
      awaitingTicketCount  = Number(awaitRow?.count ?? 0);
      resolvedTodayCount   = Number(resolvedRow?.count ?? 0);
      latestOpenTickets    = ticketRows as Record<string, unknown>[];
    } catch { /* meridian_tickets not yet created — return zeros */ }

    // Pipeline (owner = current user) — guarded in case tables don't exist
    let weightedPipeline = 0;
    let activeLeads = 0;
    let wonThisMonth = { count: 0, value: 0 };
    let pipelineByStage: Record<string, unknown>[] = [];
    ...
    try {
      ...
      pipelineByStage  = stageRows as Record<string, unknown>[];
      nextActions      = ([...actionRows] as Record<string, unknown>[]).sort(
        (a, b) => String(a.next_action_date).localeCompare(String(b.next_action_date)),
      );
    } catch { /* CRM tables not yet created — return zeros */ }
    ...
    return NextResponse.json({
      ...
      awaitingTickets:  awaitingTicketCount,
    ...
    console.error('[Meridian] dashboard GET failed:', err);
