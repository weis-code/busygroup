...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [creator] = await sql`
    SELECT c.id, c.name, c.tiktok_handle, c.is_active, c.notes, c.country, c.created_at,
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if ('country' in body)       { fields.push('country = $' + (values.push(body.country)));     }

  if (fields.length === 0) return NextResponse.json({ error: 'No fields to update' }, { status: 400 });

  values.push(creatorId);
  const [updated] = await sql.unsafe(
    `UPDATE nlca_creators SET ${fields.join(', ')} WHERE id = $${values.length} RETURNING id, name, tiktok_handle, manager_id, notes, is_active, country`,
    values as string[]
  );
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [deleted] = await sql`
    DELETE FROM nlca_creators WHERE id = ${creatorId} RETURNING id
  `;
